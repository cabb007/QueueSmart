...
function PatientLayout({ children }) {
  ...
  useEffect(() => {
    ...
    async function getNotifications() {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Unable to retrieve notifications: ${response.status}`)
        }
        ...
        setNotificationError('Unable to retrieve notifications.')
      }
    }
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3001/api/notifications/${userId}/viewed`, {
      method: 'PATCH',
    }).catch(err => console.error('Unable to mark notifications viewed:', err))
            ...
            {notificationsOpen && (
              <div className="absolute right-0 mt-2 w-96 max-w-[calc(100vw-2rem)] bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden z-50">
                <div className="px-4 py-3 border-b border-slate-100">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Recent queue updates</p>
                </div>
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map(notification => (
                      <div key={notification.id}
                        ...
                        {notification.estimatedWaitMinutes !== undefined && (
                          <p className="text-xs text-blue-700 mt-1">
                            Estimated wait: {notification.estimatedWaitMinutes} minutes
                          </p>
                        )}
