...
export default function QueueStatus() {
  ...
  useEffect(() => {
    ...
    async function getQueueStatus() {
      try {
        const response = await fetch(`http://localhost:3001/api/queue/${serviceId}`);

        if (!response.ok) {
          throw new Error(`Unable to retrieve queue status: ${response.status}`);
        }
        ...
        setEstimatedWaitTime(`${patientEntry.estimatedWaitMinutes} minutes`);
        setStatus(patientEntry.position === 1 ? "almost ready" : "waiting");
        setError("");

      } catch (error) {
        console.error("Queue status request failed:", error);
        setError(error instanceof Error ? error.message : "Unable to retrieve queue status");
      } finally {
        setLoading(false);
      }
    }
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="sm:col-span-2 bg-slate-50 border border-slate-100 rounded-xl p-5">
              ...
              <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-semibold capitalize ${statusStyles[status]}`}>
                <span className="w-2 h-2 rounded-full bg-current animate-pulse"></span>
                {status}
              </div>
              <p className="text-sm text-slate-500 mt-3">
                {status === "waiting"       && "You are checked in and currently waiting to be called."}
                {status === "almost ready"  && "You are almost ready to be seen. Please stay close."}
                {status === "served"        && "You have been served. Thank you for using QueueSmart."}
              </p>
            </div>
          </div>
        </div>
